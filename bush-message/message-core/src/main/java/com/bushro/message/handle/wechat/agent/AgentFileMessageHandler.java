 ...
 **/
@Component
public class AgentFileMessageHandler extends AbstractMessageHandler<MediaMessageDTO> {

    private final static Logger LOGGER = LoggerFactory.getLogger(AgentFileMessageHandler.class);
    ...
    public void handle(MediaMessageDTO param) {
        ...
        for (WechatWorkAgentConfig config : configs) {
            ...
            try {
                ...
                LOGGER.info("企业微信发送文件消息响应数据:{}",rsp.getErrMsg());
            } catch (Exception e) {
                LOGGER.error("企业微信发送文件失败", e);
