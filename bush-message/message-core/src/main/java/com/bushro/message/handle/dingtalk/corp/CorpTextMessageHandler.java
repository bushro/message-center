...
import com.bushro.message.dto.dingtalk.corp.TextMessageDTODing;
 ...
 **/
@Component
public class CorpTextMessageHandler extends AbstractMessageHandler<TextMessageDTODing> {
    ...
    @Override
    public void handle(TextMessageDTODing param) {
        ...
        for (DingTalkCorpConfig config : configs) {
            ...
            if (receiverUsers.size() <= 0 && StringUtils.isEmpty(param.getDeptIdList()) && false == param.isToAllUser()) {
                LOGGER.warn("请求号：{}，消息配置：{}。没有检测到接收用户或者部门", param.getRequestNo(), config.getConfigName());
                return;
            }
            ...
            MessageRequestDetail requestDetail = MessageRequestDetail.builder()
                ...
                .build();
            try {
                String token = AccessTokenUtils.getAccessToken(config.getAppKey(), config.getAppSecret());
                ...
                LOGGER.info("钉钉应用发送文本消息响应数据:{}",rsp.getBody());
            } catch (Exception e) {
                LOGGER.error("钉钉应用发送文本消息失败",e);
